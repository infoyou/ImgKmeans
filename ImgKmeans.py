...
class ImagesCluster(object):
    def __init__(self, imagedir, k):
        ...
        self._image2VectorDir = os.path.join(self._imageDir, 'image2vector')
    ...
    def _hsv2L(self, h, s, v):
        ...
        #Feature Engineering [ImageColor -> Vector]
    def _getImageColorVector(self, image):
        imgDir = os.path.join(self._imageDir, image)
        originImage = Image.open(imgDir)
        print(originImage)
    ...
    def cluster(self):
        ...
        for idx, m in enumerate(imageMembers):
            ...
            src = srcdir[idx]
            print 'clusterid:', m
            srcDir = os.path.join(self._imageDir, src)
            print 'src',srcDir
...
if __name__ == '__main__':
    ...
    imagedir =os.path.join(basedir, 'Project/ImgKmeans/images')
    imageluster =ImagesCluster(imagedir, 5)
    imageluster.cluster()
